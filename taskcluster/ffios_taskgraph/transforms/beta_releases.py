...
def resolve_beta_branch_and_head(config, tasks):
    for task in tasks:
        ...
        if not taskgraph.fast:
            heads = Git().ls_remote("origin", "refs/heads/release/v*", branches=True).splitlines()
            branches = dict(head.split('\t')[::-1] for head in heads)
            current_release_branch = max(branches, key=lambda branch: int(branch.removeprefix("refs/heads/release/v")))
